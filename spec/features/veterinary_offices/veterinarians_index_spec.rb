...
RSpec.describe 'the veterinary offices veterinarians index' do
  before(:each) do
    ...
    @vet1 = Veterinarian.create(name: 'Taylor', review_rating: 10, on_call: true,
                                 veterinary_office_id: @vet_office_1.id)
    @vet2 = Veterinarian.create(name: 'Jim', review_rating: 8, on_call: true, veterinary_office_id: @vet_office_1.id)
  ...
  it 'lists all the veterinarians associated with the office, with their attributes' do
    ...
    within "#veterinarian-#{@vet1.id}" do
      expect(page).to have_content(@vet1.name)
      expect(page).to have_content(@vet1.review_rating)
      expect(page).to have_content(@vet_office_1.name)
    end

    within "#veterinarian-#{@vet2.id}" do
      expect(page).to have_content(@vet2.name)
      expect(page).to have_content(@vet2.review_rating)
    end
  ...
  it 'displays a link to edit each veterinarian' do
    ...
    expect(page).to have_link("Edit #{@vet1.name}")
    expect(page).to have_link("Edit #{@vet2.name}")

    click_link("Edit #{@vet1.name}")
    expect(page).to have_current_path("/veterinarians/#{@vet1.id}/edit")
  end

  it 'displays a link to delete each veterinarian' do
    visit "/veterinary_offices/#{@vet_office_1.id}/veterinarians"

    expect(page).to have_link("Delete #{@vet1.name}")
    expect(page).to have_link("Delete #{@vet2.name}")

    click_link("Delete #{@vet1.name}")

    expect(page).to have_current_path('/veterinarians')
    expect(page).to_not have_content(@vet1.name)
  end
